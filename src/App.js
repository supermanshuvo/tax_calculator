...
function App() {
  ...
  const calculatePayableTax = (
    totalTaxableIncome = 700000,
    gender = "male"
  ) => {
    ...
    <div className="App">
      {!formSubmitted ? (
        <>
          <Header />
          <InComeDetails
            handleStates={(formData, totalTaxableIncome, provFund, submitted) =>
              handleSubmit(formData, totalTaxableIncome, provFund, submitted)
            }
          />
          <Footer />
        </>
